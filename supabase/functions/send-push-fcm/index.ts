...
async function getAccessToken(): Promise<string> {
    // Normalizar a chave privada - pode vir com \n literal ou com quebras de linha reais
    let privateKey = FIREBASE_PRIVATE_KEY;

    // Se a chave tem \\n literal (escaped), substituir por quebra de linha real
    ...
    console.log('   Começa com BEGIN PRIVATE KEY:', privateKey.includes('-----BEGIN PRIVATE KEY-----'));
    console.log('   Termina com END PRIVATE KEY:', privateKey.includes('-----END PRIVATE KEY-----'));
    ...
    const headerIndex = privateKey.indexOf(pemHeader);
    const footerIndex = privateKey.indexOf(pemFooter);

    if (headerIndex === -1 || footerIndex === -1) {
        console.error('❌ Chave privada inválida - não contém BEGIN/END PRIVATE KEY');
    ...
    try {
        ...
        console.log('🔐 Solicitando access token do Google OAuth...');
        ...
        console.log('   OAuth response:', tokenText.substring(0, 500));

        let tokenData;
        try {
            tokenData = JSON.parse(tokenText);
        } catch (e) {
            console.error('❌ Falha ao parsear resposta OAuth:', tokenText);
...
async function sendFCMNotification(
    ...
    console.log('📤 Enviando FCM:', { title, body, hasData: !!data });

    // Preparar dados com title e body incluídos para redundância
    const notificationData = {
        ...data,
        title: title,
        body: body,
        type: data?.type || 'marketing',
        url: data?.url || '/admin/marketing',
    ...
    const message = {
        message: {
            token: fcmToken,
            // NÃO incluir notification top-level - isso faz o Firebase exibir notificação genérica
            // Em vez disso, usar apenas webpush.notification para controle total

            // WebPush specific configuration - isso permite controle total da notificação
            webpush: {
                notification: {
                    title: title,
                    body: body,
                    ...
                    tag: 'syshair-marketing-' + Date.now(),
                    actions: [
                ...
                // Data payload - disponível no Service Worker
                data: Object.fromEntries(
                    Object.entries(notificationData).map(([k, v]) => [k, String(v)])
                )
            },
            // Data payload top-level para compatibilidade e para o Service Worker
            data: Object.fromEntries(
                Object.entries(notificationData).map(([k, v]) => [k, String(v)])
            ),
            android: {
    ...
    console.log('📦 Payload FCM:', JSON.stringify(message, null, 2));
