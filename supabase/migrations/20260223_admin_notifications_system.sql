...
BEGIN
  ...
  VALUES (
    ...
    to_char(NEW.appointment_date, 'DD/MM/YYYY') || ' às ' || to_char(NEW.appointment_date, 'HH24:MI'),
    NEW.client_name,
    NEW.client_phone,
...
BEGIN
  ...
  IF NEW.appointment_date != OLD.appointment_date THEN
    ...
    VALUES (
      ...
      to_char(OLD.appointment_date, 'DD/MM/YYYY HH24:MI') || ' para ' ||
...
CREATE TRIGGER on_appointment_rescheduled
  AFTER UPDATE ON public.appointments
  FOR EACH ROW
  WHEN (NEW.appointment_date IS DISTINCT FROM OLD.appointment_date)
